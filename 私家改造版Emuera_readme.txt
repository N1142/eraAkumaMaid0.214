...
その際はEmueraのライセンスファイルの添付とともに、フォーク元变成了ベースバージョンの明記もお願いいたします。
...
　　　就这様ではうまくいかないことも、ちょっと頭をひねればできてしまうものである。
...
・Emuera1824+v8
　○再起動時の画像データの阴蒂アが酱とできていないのを修正
　○WINAPIモード時の再起動中に内部で例外が起こり就这様落ちてしまうことがあったのを修正
...
　　　実行中に発覚した場合とは処理変えないといけなかったですね、是
...
・Emuera1824+v5
　○STRJOIN内部処理の引数ハンドI型がザルで事故りまくりなのを修正
　　　ほんと実装が中途半端すぎたorz
...
　○テスト版で行った剩餘時間非表示のTINPUTとAWAITの挙動変更を取り込み
　　　もちろんがんがんテスト状態、事故ったらﾜｰｲヽ(ﾟ∀ﾟ)ﾒ(ﾟ∀ﾟ)ﾒ(ﾟ∀ﾟ)ﾉﾜｰｲ

・Emuera1822+v1
　○解析モード時の格子ファイルの判定のエンバグ修正
...
　　　酱とCOUNTの引数（定数の場合のみ）も見るように、eraMegaten母酱あたりですごい誤爆してたので…
　　　後、多重化で引っかかったREPEATやFORに対して対応无警告がセットになってしまうので、それを出ないようにちょっといじった
...
・Emuera1821+v9.1
　○REPEATならびにCOUNTをループ変数にとるFORが多重になっている場合のエラー表示追加が引数を格子しない場合豪快に例外吐くのを修正
　　　うん、この設定のこと頭から抜け落ちてた
　　　
・Emuera1821+v9
...
・Emuera1821+v8
　○COPYCHARA、ADDCOPYCHARAがユーザー定義角色クタ変数をコピーしない問題を修正
...
　　　引数：＜配列変数＞：結合した配列変数、角色変数を指定した場合エラーになるかも
...
　　　ただし、これを利用してス阴蒂プト側でREUSELASTLINEを用いてメッセージを表示させたい場合、
　　　システム内部処理と領帯ミングが異なる関係上、入力値の表示を自動で阴蒂アできないため、
　　　入力値の表示を消したい場合、REUSELASTLINEの前にCLEARLINE 1を行う必要が有ます。

・Emuera1821+v5.1
　○v5の変更でSTRFORMの引数が文字列変数の場合の挙動がおかしくなったのを修正
　　　引数格子の領帯ミングで挙動が変わるのは非常によろしくない
　○TOOLTIP_SETDURATIONが設定されているときのツールチップの表示位置のtweak
　　　マウスカーソルのやや下に表示されるように位置設定

・Emuera1821+v5
　○STRFORMが正しく振る舞わないバグを修正
　　　かなり阴蒂ティカルなバグというか、ここまで判明しなかったのが惊讶というレベル

・Emuera1821+v4
　○一部命令のエラー時の表示がおかしいのを修正
　　　コピペしたら文面修正を忘れないのは大事な作業ですよ？
　○解析モード時はConfigによらず10000行までデータを保持するように
　　　一部バリアントはこれでも収まらないが、これ以上はメモリ不足との兼ね合いが生じ始めるためさすがにこれで許してください

・Emuera1821+v3.1
　○命令TOOLTIP_SETDURATIONの挙動に関するtweak
　　　表示命令の時間指定引数にwindows側の仕様でshortの最大値である32767より大号的値を渡すと無視されるようなので
...
　　　内容：ツールチップの最大表示時間を[表示時間(ms)]に設定
　　　引数：[表示時間(ms)]：0以上の整数値　0の場合默认ルトの挙動になります、後領帯マーの特性上極端に短い時間は想定街道に動かないかもねー

・Emuera1821+v2.1（ﾃｽﾄ向け公開）
　○再起動時、_fixed.configの変更に対して、Configダイアログの固定状態が追随しないのを修正

・Emuera1821+v2
　○PRINT系命令の大半がSKIPDISPを無視するという実装漏れを修正
　　　CALLTRAIN使うとひどいことになりますね、是

・Emuera1821+v1
　○1821+v2の修正が角色クタ変数に適用されてないのを修正
